...
func runHook(args []string, stdin io.Reader, stdinIsTerminal bool, stdout, stderr io.Writer) {
	...
	invocation := &hookInvocation{
		...
		client: &http.Client{CheckRedirect: refuseMCPRedirect}, stdout: stdout, diagnostics: diagnostics, pid: os.Getpid(),
	}
	invocation.run()
}
...
func (hook *hookInvocation) headers(request *http.Request) {
	request.Header.Set("X-October-Caller-Pid", fmt.Sprint(hook.pid))
